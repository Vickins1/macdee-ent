...
(function ($) {
  ...
  var $dark_mode_checkbox = $('<input />', {
    type: 'checkbox',
    value: 0,
    checked: $('body').hasClass('dark-mode'),  // Set checkbox checked based on the body's dark mode state
    class: 'mr-1'
  }).on('click', function () {
    if ($(this).is(':checked')) {
      $('body').addClass('dark-mode');  // Add dark mode to body
    } else {
      $('body').removeClass('dark-mode');  // Remove dark mode from body
    }
  });
  
  // Ensure light mode is default (no dark-mode class on body)
  if ($('body').hasClass('dark-mode')) {
    $('body').removeClass('dark-mode'); // Remove dark mode class to enforce light mode
    $dark_mode_checkbox.prop('checked', false);  // Ensure the checkbox is unchecked for light mode
  } else {
    $dark_mode_checkbox.prop('checked', false); // Checkbox remains unchecked if light mode is active
  }
